...
- **Home Latest:** agent prepends the new id to `FEATURED_EXPLORATION_IDS` in `src/views/homeCatalog.ts` (the previous third Latest moves to the Catalog row). Home cards use `ExplorationPreviewC` (cover + core question), not the full widget chrome. **Catalog pills** (`ExplorationMetaPills`) appear on home/topic preview cards and on the exploration shell (`ExplorationC`) below the title: topic → topic hub, timeline → `/all?timeline=…`, MCP → `mcp-docs/use/eips/eip-NNNN.html` when `mcpDocsStatus` is runnable or planned-module (label is always **MCP**, not Runnable/Planned).
