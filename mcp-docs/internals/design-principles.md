...
- **Stateless / BYOS** — Callers supply bytecode, transaction fields, and state overrides. No archive node, no mainnet sync.
...
- **Intent-driven MCP tools** — Tools match use cases (run bytecode, run transaction, generate BAL, …), not raw library APIs one-to-one.
...
- **Query shapes, not library APIs** — generic verbs (`simulate`, `transaction`, `generate`, `probe`); structured results.
